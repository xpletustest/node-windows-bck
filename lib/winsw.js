const {
    isValidOnFailure,
    isValidFailureDelay} = require("./validation");
...
module.exports = {
  ...
  generateXml: function(config)
  {
    ...
    // optionally set resetFailure
    if (config.resetFailure && isValidFailureDelay(config.resetFailure)) {
        xml.push({
            resetfailure: config.resetFailure
        });
    }

    // optionally add onFailure elements
    if (config.onFailure && isValidOnFailure(config.onFailure)) {
        for (onFailure of config.onFailure) {
            const {action, delay} = onFailure;
            xml.push({
                onfailure: {_attr: { action, delay }}
            });
        }
    }
  ...
  createExe: function(exeName,dir,callback) {
    ...
    var exeOrigin = p.join(__dirname,'..','bin','winsw','winsw.exe'),
        ...
        exeDest = p.join(dir,exeName+'.exe'),
        cfgDest = p.join(dir,exeName+'.exe.config'),

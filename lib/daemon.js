          ...
          fs.writeFile(path.resolve(dir,me.exeName+'.xml'), me._xml, function(){
            ...
            winsw.createExe(me.exeName, dir, function(){
              me.execute('"'+path.resolve(dir, me._exe)+'" install', function(){
                sleep(2);
                // should hide password from the xml file after installation ?
                if (me.logOnAs.password !== null && me.logOnAs.password !== "") {
                  me.logOnAs.password = null;
                  // re-write the xml configuration file
                  fs.writeFile(path.resolve(dir,me.exeName+'.xml'),me._xml, function(){
                    me.emit('install');
                  });
                }
